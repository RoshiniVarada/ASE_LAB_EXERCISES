...
export class ContactPage {
  ...
  getKnowledgeGraph(){
    ...
    this.http.get('https://kgsearch.googleapis.com/v1/entities:search?query=' + this.searchWord +
      '&key=AIzaSyBVDjswXIJzRT-hUMRB56IqPCmBXVmwuZw&limit=1&indent=True').subscribe((data: any)=>{
      console.log(data);
  ...
  getSearchedImages(){
    ...
    this.http.get('https://www.googleapis.com/customsearch/v1?q=' + this.searchImg +
      '&key=AIzaSyBVDjswXIJzRT-hUMRB56IqPCmBXVmwuZw&imgSize=medium&searchType=image&fileType=jpg&cx=005069354886028579975:vpkirx0dbmp').subscribe((data: any)=>{
